...
`docs/` 是可直接部署到 GitHub Pages 的静态展示版，采用与 FlowFuse Dashboard 一致的深色仪表盘布局。它不再生成模拟电压，默认显示“等待第一条真实采样数据”。如需接入外部数据桥接，可调用 `window.pushBatterySample({timestamp, channels: {CH1, CH2, CH3}, statusText, cycleCount})`。
